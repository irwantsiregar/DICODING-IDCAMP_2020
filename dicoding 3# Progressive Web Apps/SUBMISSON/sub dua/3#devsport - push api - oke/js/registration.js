...
function registerServiceWorker() {
  return navigator.serviceWorker.register('../service-worker.js')
    ...
    .catch( error => console.error('Opps.. service worker registration has failed.', error));
}
    ...
    Notification.requestPermission().then( result => {
      ...
      if ('PushManager' in window) {
        navigator.serviceWorker.getRegistration().then( registration => {
          ...
          .then( subscribe => {
              console.log('Successfully subscribe with an endpoint: ', subscribe.endpoint);
              console.log('Successfully subscribe with an p256dh key: ', btoa(String.fromCharCode.apply(null, new Uint8Array(subscribe.getKey('p256dh'))) ));
              console.log('Successfully subscribe with an auth key: ', btoa(String.fromCharCode.apply(null, new Uint8Array(subscribe.getKey('auth'))) ));
          })
