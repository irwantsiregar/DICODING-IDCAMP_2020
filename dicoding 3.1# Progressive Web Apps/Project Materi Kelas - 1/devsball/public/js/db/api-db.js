...
function getSavedFavorites() {
  getAll().then(function(teams) {
    if (teams === null || teams === false || teams < 1) {
      const emptyFavHTML = emptyFavorites();

      document.getElementById("teams").innerHTML = emptyFavHTML;         
    } else {
      let teamsHTML = '';
...
function getSavedFavoritesById() {
  ...
  getOneById(idParam).then(function(data) {
    let teamHTML = `             
...
function emptyFavorites() {
  ...
  <li class="collection-item avatar">
    <div class="row">
      <h6 class="card-title text-grey text-darken-1">You haven't added a team to your favorites, please <a href="index.html#standings" class=""><em>click here.<em></a></h6>
    </div>
  </li>
  `;
  return emptyHTML;
